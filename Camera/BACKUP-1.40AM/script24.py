# script24.py
# Uses ORIGINAL Homography matrix. Assumes camera is RIGHT-SIDE UP.
# Based on successful calibration after setting FLIP_CAMERA_INPUT_VERTICALLY = False
...
HOMOGRAPHY_FILE = "homography_matrix.npz" # Load the NEW one generated
...
# --- FIXES / SETTINGS ---
# Settings used for the SUCCESSFUL homography calibration
FLIP_CAMERA_INPUT_VERTICALLY = False # Set to False as per successful test
FLIP_CAMERA_INPUT_HORIZONTALLY = False # Keep False

# Output flips likely NOT needed, start with False
FLIP_DEBUG_VERTICALLY = False
FLIP_OUTPUT_VERTICALLY = False

UNDISTORT_ALPHA = 1.0 # Keep consistent with calibration
...
# --- Load Homography Matrix (ORIGINAL H) ---
try:
    with np.load(HOMOGRAPHY_FILE) as data:
        H_matrix_to_use = data['homography'] # Load the original matrix
        if H_matrix_to_use is None or H_matrix_to_use.shape != (3, 3): raise ValueError("Invalid Matrix loaded")
    print(f"ORIGINAL Homography loaded from '{HOMOGRAPHY_FILE}'.\nMatrix H (Cam->Proj):\n", H_matrix_to_use)
...
print("\nStarting main loop using ORIGINAL Homography. Press 'q' to quit.", flush=True)
...
try:
    while True:
        ...
        # Apply Input Flips (Should be just vertical if needed)
        ...
        if results.multi_hand_landmarks:
            ...
            for hand_no, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                if 0 <= cx < w and 0 <= cy < h:
                    ...
                    # USE THE ORIGINAL MATRIX FOR TRANSFORMATION
                    proj_pt_transformed = cv2.perspectiveTransform(cam_pt, H_matrix_to_use) # Using ORIGINAL H
        ...
        # === DISPLAY OUTPUTS ===
        # ... (FPS and flip checks remain the same) ...
