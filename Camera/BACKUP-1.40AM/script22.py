# script22.py
# MAIN APP: Loads calculated homography and projects TRANSFORMED red dot.
# Debug view shows camera + GREEN dot (original coords).
# Projector shows black + RED dot (transformed coords).
...
# >>> Ensure this points to the file just created <<<
HOMOGRAPHY_FILE = "homography_matrix.npz"
...
# --- FIXES / SETTINGS ---
FLIP_CAMERA_INPUT_VERTICALLY = True # Or False, set based on your camera mount
FLIP_CAMERA_INPUT_HORIZONTALLY = False # Should be False if handedness detection is correct

# Output flips likely NOT needed now, start with False
FLIP_DEBUG_VERTICALLY = False
FLIP_OUTPUT_VERTICALLY = False

# Undistortion setting used during homography calibration (keep consistent)
UNDISTORT_ALPHA = 1.0 # Was 1.0 in calibrate_homography

SHOW_DEBUG_WINDOW = True
DEBUG_WINDOW_WIDTH = 960; DEBUG_WINDOW_HEIGHT = 540

# --- Styles ---
PROJECTED_DOT_COLOR=(0,0,255); PROJECTED_DOT_RADIUS=15 # Red dot on projector
DEBUG_LANDMARK_COLOR=(0,255,0); DEBUG_DOT_RADIUS=10 # Green dot on debug
# --- End Configuration ---
...
try:
    with np.load(CAMERA_CALIBRATION_FILE) as data:
        ...
        img_size_calib=data['img_size']; print("Camera calibration data loaded.", flush=True)
except Exception as e: print(f"Error loading camera calib: {e}", flush=True); exit()

# --- Load Homography Matrix ---
try:
    with np.load(HOMOGRAPHY_FILE) as data:
        H_matrix = data['homography'] # Load the homography matrix
        if H_matrix is None or H_matrix.shape != (3, 3): raise ValueError("Invalid Matrix")
...
# --- Calculate Undistortion Maps (using same alpha as calibration) ---
...
prev_time = 0; hand_positions = []; projected_positions = [] # For storing coords if needed
            ...
            for hand_no, hand_landmarks in enumerate(results.multi_hand_landmarks):
                ...
                if 0 <= cx < w and 0 <= cy < h:
                    ...
                    if proj_pt_transformed is not None and proj_pt_transformed.shape == (1,1,2):
                        ...
                        # Draw RED dot on PROJECTOR CANVAS using TRANSFORMED coords
                    ...
                    # Draw GREEN dot/skeleton on DEBUG VIEW using ORIGINAL coords
