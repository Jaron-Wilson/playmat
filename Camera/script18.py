# script18.py
# CORRECTED: Added flag/code to flip CAMERA INPUT HORIZONTALLY.
# This should fix the handedness detection and projection mirroring.
...
# --- FIXES / SETTINGS ---
# >>> If Camera Hardware is mounted upside-down <<<
FLIP_CAMERA_INPUT_VERTICALLY = True # Or False, based on your physical setup

# >>> NEW: Set True if MediaPipe detects wrong handedness (e.g., sees Left as Right) <<<
# >>> This flips the camera input horizontally BEFORE processing <<<
FLIP_CAMERA_INPUT_HORIZONTALLY = True # <<< TRY THIS!

# >>> Set these back to False - we are fixing the INPUT now <<<
FLIP_PROJECTOR_OUTPUT_HORIZONTALLY = False
FLIP_DEBUG_VERTICALLY = False
FLIP_OUTPUT_VERTICALLY = False # Use projector/Windows settings for vertical flip first
...
try:
    while True:
        ...
        if FLIP_CAMERA_INPUT_VERTICALLY:
            frame = cv2.flip(frame, 0) # Vertical flip first (if needed)
        if FLIP_CAMERA_INPUT_HORIZONTALLY:
            frame = cv2.flip(frame, 1) # THEN Horizontal flip (if needed)
        # -------------------------

        # 1. Undistort (using potentially flipped frame)
        undistorted_frame = cv2.remap(frame, mapx, mapy, cv2.INTER_LINEAR); h, w, _ = undistorted_frame.shape

        # 2. Prep for MediaPipe
        # IMPORTANT: Process the *undistorted* frame now
        ...
        # === PROCESS HANDS & DRAW ===
        current_hand_positions = []; detected_handedness = []
        ...
        # 6. Prepare PROJECTOR frame (Monitor 2) - Apply flips if needed
        ...
        if FLIP_PROJECTOR_OUTPUT_HORIZONTALLY: frame_to_display_projector = cv2.flip(frame_to_display_projector, 1) # Should be False now

        # 7. Prepare DEBUG frame (Monitor 1) - Apply flips if needed
